---
type: source
created: 2026-07-11
tags: [wiki, source, Anthropic, alignment, tool, Petri, open-source, year-2026]
source-type: 工具发布
lang: en
source-url: https://www.anthropic.com/research/donating-open-source-petri
published: 2026-05-07
raw:
  - '[原文](/clippings/2026-05-07 Anthropic - Donating our open-source alignment tool)'
related:
  - '[[concepts/Anthropic-Research-2026H1]]'
  - '[[entities/Petri]]'
---

# Donating our open-source alignment tool

> Anthropic · 2026-05-07 · [原文](https://www.anthropic.com/research/donating-open-source-petri)

## TL;DR

Anthropic 把内部对齐审计工具 **Petri** 开源捐赠。目的是让整个 AI 安全生态都能用同一套工具做审计，加速 alignment 研究。

## 关键论点

- **开源审计工具** = 加速对齐领域的公共基础设施
- 从"公司内部工具"到"社区共用工具"的战略转换
- 与 [[sources/138-Anthropic-Off-Switch-Dual-Use]] 一起代表 Alignment 从研究到工程的成熟化

## 与本 wiki 关联

- 相关实体：[[entities/Petri]]
- 组合：[[sources/148-Anthropic-Teaching-Claude-Why]]

## 一句话总结

> **Petri 开源 = Anthropic 让 alignment 工具变成公共基础设施。**

## 原文链接

- 本地剪藏：[原文](/clippings/2026-05-07 Anthropic - Donating our open-source alignment tool)
- 官方链接：<https://www.anthropic.com/research/donating-open-source-petri>
