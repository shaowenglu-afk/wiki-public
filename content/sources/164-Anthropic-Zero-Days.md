---
type: source
created: 2026-07-11
tags: [wiki, source, Anthropic, red-team, security, zero-day, year-2026]
source-type: 研究博客
lang: en
source-url: https://www.anthropic.com/research/zero-days
published: 2026-02-05
raw:
  - '[[clippings/2026-02-05 Anthropic - LLM-discovered 0 days|原文全文]]'
related:
  - '[[concepts/Anthropic-Research-2026H1]]'
---

# LLM-discovered 0-days

> Anthropic Frontier Red Team · 2026-02-05 · [原文](https://www.anthropic.com/research/zero-days)

## TL;DR

首次公开报告 **LLM 独立发现的 0-day 漏洞**：不是复现已知的 CVE，是 **AI 找到人类未知的新漏洞**。是 Red Team 从"评估能力"进入"实证发现"的分水岭。

## 关键论点

- 首次实证 **LLM 找到人类未知漏洞**（不是 N-day 加速，是原创发现）
- 意味着 AI 已经在部分安全场景成为**新型研究员**
- 与 [[sources/142-Anthropic-N-Day-Exploits]] 形成 0-day / N-day 完整视角

## 与本 wiki 关联

- 组合：[[sources/142-Anthropic-N-Day-Exploits]] · [[sources/145-Anthropic-Attack-Navigator]]

## 一句话总结

> **AI 找到了人类未知的 0-day —— 安全领域从此有新型研究员。**

## 原文链接

- 本地剪藏：[[clippings/2026-02-05 Anthropic - LLM-discovered 0 days|原文全文]]
- 官方链接：<https://www.anthropic.com/research/zero-days>
