---
type: source
created: 2026-07-11
tags: [wiki, source, Anthropic, agent, long-running, scientific-computing, harness, year-2026]
source-type: 研究博客
lang: en
source-url: https://www.anthropic.com/research/long-running-Claude
published: 2026-03-23
raw:
  - '[[clippings/2026-03-23 Anthropic - Long-running Claude for scientific computing|原文全文]]'
related:
  - '[[concepts/Anthropic-Research-2026H1]]'
  - '[[concepts/Harness-Engineering]]'
---

# Long-running Claude for scientific computing ⭐

> Anthropic Science · 2026-03-23 · [原文](https://www.anthropic.com/research/long-running-Claude)

## TL;DR

**长跑 Claude** 用于科学计算：让 Claude 做**跨天 / 跨周**的持续任务（模拟、参数扫描、迭代实验）。这是 [[concepts/Harness-Engineering]] 的**Anthropic 官方一手实证**。

## 关键论点

- **长跑 Agent** 的工程范式：任务持久化 + checkpoint + 恢复 + 观测
- 科学计算是长跑最自然的应用场景
- **直接对应本 wiki 的 Harness Engineering 主线**

## 与本 wiki 关联

- **上承**：[[concepts/Harness-Engineering]] · [[concepts/Brain-Hands-Decoupling]]
- **组合**：[[sources/141-Anthropic-Project-Fetch-Phase-Two]] · [[sources/156-Anthropic-Vibe-Physics]]

## 一句话总结

> **长跑 Claude 用于科学计算 = Harness Engineering 的 Anthropic 官方站台。**

## 原文链接

- 本地剪藏：[[clippings/2026-03-23 Anthropic - Long-running Claude for scientific computing|原文全文]]
- 官方链接：<https://www.anthropic.com/research/long-running-Claude>
